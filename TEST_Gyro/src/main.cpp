...
unsigned long int measurementLastTime = 0;
...
void setup()
{
  ...
  while(count > 0){
    ...
    count--;
    BMX055_Accl();
    xAccl = xAccl - xDefaultAccl;
    measurementTime = millis();

    distance += ((xAccl * ((double)(measurementTime - measurementLastTime) * ((double)(measurementTime - measurementLastTime) / 1000)) / 2);
